...
in the `cm2` conda environment. The local Flask server reads that installed
bundle at runtime; the Sites deployment vendors the exact same bytes so the
public Worker does not depend on Python.
